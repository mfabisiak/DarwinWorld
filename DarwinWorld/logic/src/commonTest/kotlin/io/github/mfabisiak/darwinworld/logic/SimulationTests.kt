...
import io.github.mfabisiak.darwinworld.logic.config.ProductionConfig
...
class SimulationTest {
    ...
    fun simulationShouldInitializeWithCorrectNumberOfEntities() {
        ...
        val map = simulation.state.value.worldMap
    ...
    fun animalsShouldLoseEnergyOverMultipleDays() {
        ...
        val animal = simulation.state.value.worldMap.animals.values.first()
        // then
    ...
    fun populationShouldGrowOnCrowdedMap() {
        ...
        repeat(maxDays) {
            simulation.simulateDay()
            if (simulation.state.value.worldMap.animals.size > 4) {
                return@repeat
            }
        }
